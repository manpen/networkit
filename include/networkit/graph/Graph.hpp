...
#include <networkit/graph/details/CallbackDispatcher.hpp>
...
inline void Graph::forOutEdgesOfImpl(node u, L handle) const {
    for (index i = 0; i < outEdges[u].size(); ++i) {
        ...
        if (useEdgeInIteration<graphIsDirected>(u, v)) {
            GraphDetails::callEdgeHandler<L>(handle, u, v, getOutEdgeWeight<hasWeights>(u, i),
    ...
    if (graphIsDirected) {
        for (index i = 0; i < inEdges[u].size(); i++) {
            ...
            if (useEdgeInIteration<true>(u, v)) {
                GraphDetails::callEdgeHandler<L>(handle, u, v, getInEdgeWeight<hasWeights>(u, i),
...
#pragma omp parallel for reduction(+ : sum)
    for (omp_index u = 0; u < static_cast<omp_index>(z); ++u) {
        for (index i = 0; i < outEdges[u].size(); ++i) {
            ...
            if (useEdgeInIteration<graphIsDirected>(u, v)) {
                sum += GraphDetails::callEdgeHandler<L>(handle, u, v,
...
void Graph::BFSfrom(const std::vector<node> &startNodes, L handle) const {
    ...
    do {
        ...
        q.pop();
        // apply function
        GraphDetails::callBFSHandler(handle, u, dist);
...
std::pair<count, count> Graph::removeAdjacentEdges(node u, Condition condition, bool edgesIn) {
    ...
    for (index vi = 0; vi < edges_.size();) {
        ...
        if (GraphDetails::callEdgeHandler(condition, u, v, weight, edgeId)) {
