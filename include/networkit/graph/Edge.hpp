...
/// Defines and implements an interface for topology related methods
template<typename T>
class EdgeTopology {
public:
// manipulate
...
class Edge : public details::EdgeTopology<Edge> {
public:
...
class WeightedEdge : public details::EdgeTopology<WeightedEdge> {
public:
    // we keep the names first and second for compatibility with std::pair<node, node>
    node first;  ///< Tail node
    node second; ///< Head node
